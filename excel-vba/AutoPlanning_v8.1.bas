'===============================================================================
' 외주처 생산계획 자동화 시스템 - VBA 매크로 v8.1
'===============================================================================
' [v8.1 변경사항]
' - 이동일을 사용자 입력으로 변경 (기존: 자동 계산)
' - 순산 로직: 이동일 -> 생산완료 -> 납기 체크
' - 납기준수 여부 컬럼 추가 (OK/위험/불가)
' - 세트품 1코드: 납기 빈칸이면 9코드 납기 자동 적용
' - Raw데이터 컬럼 재구성 (A~K 입력, L~R 자동)
'===============================================================================
...
Public Const RAW_COL_TRANSFER As Integer = 4      ' D: 이동일 (사용자 입력)
Public Const RAW_COL_DUEDATE As Integer = 5       ' E: 납기
Public Const RAW_COL_PROCESS As Integer = 6       ' F: 공정명
Public Const RAW_COL_SPECIAL As Integer = 7       ' G: 특수공정
Public Const RAW_COL_VENDOR As Integer = 8        ' H: 지정업체
Public Const RAW_COL_URGENCY As Integer = 9       ' I: 긴급도
Public Const RAW_COL_STATUS As Integer = 10       ' J: 상태
Public Const RAW_COL_REMARKS As Integer = 11      ' K: 비고

' === 자동 생성 영역 (L~R) ===
Public Const RAW_COL_CLIENT As Integer = 12       ' L: 고객사코드 (자동)
Public Const RAW_COL_ASSIGNED As Integer = 13     ' M: 배정업체 (자동)
Public Const RAW_COL_PRODSTART As Integer = 14    ' N: 생산시작일 (자동)
Public Const RAW_COL_PRODEND As Integer = 15      ' O: 생산완료일 (자동)
Public Const RAW_COL_DUECHECK As Integer = 16     ' P: 납기준수 (자동)
Public Const RAW_COL_ORIGINAL As Integer = 17     ' Q: 최초배정업체 (자동)
Public Const RAW_COL_CHANGED As Integer = 18      ' R: 업체변경 (자동)
...
Public Type ProductionItem
    ...
    Quantity As Long
    TransferDate As Date          ' 이동일 (사용자 입력)
    DueDate As Date               ' 납기
    ProcessType As String
    SpecialProcess As String
    DesignatedVendor As String
    Urgency As String
    Status As String
    Remarks As String
    ...
    DueCheckStatus As String      ' 납기준수 (OK/위험/불가)
    ...
    SetTotalQty As Long
    SetLeaderRow As Long          ' 세트품 리더(9코드) 행번호
End Type
...
' 색상 팔레트
'===============================================================================
Private Const COLOR_PRIMARY As Long = 3955058
Private Const COLOR_PRIMARY_LIGHT As Long = 15724527
Private Const COLOR_ACCENT As Long = 15124704
Private Const COLOR_SUCCESS As Long = 7978352
Private Const COLOR_WARNING As Long = 7053567
Private Const COLOR_DANGER As Long = 5765887
Private Const COLOR_BG_WHITE As Long = 16777215
Private Const COLOR_BG_GRAY As Long = 15921906
Private Const COLOR_TEXT_DARK As Long = 2500134
Private Const COLOR_TEXT_GRAY As Long = 8421504
...
Private Sub CreateRawDataSheet()
    ...
    headers = Array("품번", "품명", "수량", "이동일", "납기", "공정명", "특수공정", _
                    "지정업체", "긴급도", "상태", "비고", _
                    "고객사코드", "배정업체", "생산시작일", "생산완료일", "납기준수", "최초배정업체", "업체변경")
    ...
    ws.Columns("D").ColumnWidth = 12  ' 이동일
    ws.Columns("E").ColumnWidth = 12  ' 납기
    ws.Columns("F").ColumnWidth = 12  ' 공정명
    ws.Columns("G").ColumnWidth = 10  ' 특수공정
    ws.Columns("H").ColumnWidth = 12  ' 지정업체
    ws.Columns("I").ColumnWidth = 8   ' 긴급도
    ws.Columns("J").ColumnWidth = 10  ' 상태
    ws.Columns("K").ColumnWidth = 15  ' 비고
    ws.Columns("L").ColumnWidth = 10  ' 고객사코드
    ws.Columns("M").ColumnWidth = 12  ' 배정업체
    ws.Columns("N").ColumnWidth = 12  ' 생산시작일
    ws.Columns("O").ColumnWidth = 12  ' 생산완료일
    ws.Columns("P").ColumnWidth = 10  ' 납기준수
    ws.Columns("Q").ColumnWidth = 12  ' 최초배정업체
    ws.Columns("R").ColumnWidth = 10  ' 업체변경
    
    ' 드롭다운 설정 (공정명)
    With ws.Range("F2:F1000").Validation
        .Delete
        .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, _
             Formula1:="포장,충전,충포장,포장/충포장,충전/충포장"
    End With
    
    ' 드롭다운 설정 (특수공정)
    With ws.Range("G2:G1000").Validation
        .Delete
        .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, _
             Formula1:="수축,교반,고주파"
    End With
    ...
    ' 고객사코드 자동 생성 수식 (L열)
    ws.Range("L2").Formula = "=IF(A2="""","""",MID(A2,2,3))"
    ws.Range("L2").AutoFill Destination:=ws.Range("L2:L100")
...
Private Sub ApplyConditionalFormatting(ws As Worksheet)
    ...
    ' 납기준수별 서식 (P열)
    Set rng = ws.Range("P2:P1000")
    rng.FormatConditions.Delete
    
    ' OK - 녹색
    With rng.FormatConditions.Add(Type:=xlCellValue, Operator:=xlEqual, Formula1:="=""OK""")
        .Interior.Color = RGB(209, 250, 229)
        .Font.Color = RGB(6, 95, 70)
        .Font.Bold = True
    End With
    ...
    ' 위험 - 주황
    With rng.FormatConditions.Add(Type:=xlCellValue, Operator:=xlEqual, Formula1:="=""위험""")
        .Interior.Color = RGB(255, 237, 213)
        .Font.Color = RGB(194, 65, 12)
        .Font.Bold = True
    End With
    ...
    ' 불가 - 빨강
    With rng.FormatConditions.Add(Type:=xlCellValue, Operator:=xlEqual, Formula1:="=""불가""")
        .Interior.Color = RGB(254, 226, 226)
        .Font.Color = RGB(185, 28, 28)
        .Font.Bold = True
    End With
...
'===============================================================================
' 샘플 데이터 추가 (v8.1 - 이동일 포함)
'===============================================================================
Private Sub AddSampleData(ws As Worksheet)
    Dim data As Variant
    ' 컬럼: 품번, 품명, 수량, 이동일, 납기, 공정명, 특수공정, 지정업체, 긴급도, 상태, 비고
    data = Array( _
        Array("1OLV01", "샴푸 500ml", 15000, "2026-01-20", "2026-01-25", "충포장", "", "", "", "", ""), _
        Array("2AMR05", "로션 200ml", 8000, "2026-01-19", "2026-01-24", "충전", "", "", "급함", "", "수출건"), _
        Array("9CJO12", "크림 50g", 5000, "2026-01-18", "2026-01-22", "포장", "교반", "", "긴급", "", "홈쇼핑"), _
        Array("9AMR10", "세트A-포장", 5000, "2026-01-21", "2026-01-26", "포장/충포장", "", "", "", "", ""), _
        Array("1AMR10", "세트A-충전1", 5000, "2026-01-21", "", "충전/충포장", "", "", "", "", ""), _
        Array("1AMR11", "세트A-충전2", 3000, "2026-01-21", "", "충전/충포장", "", "", "", "", ""), _
        Array("1LGH03", "에센스 30ml", 3000, "2026-01-22", "2026-01-28", "충포장", "", "리니어", "", "", ""), _
        Array("9EMT07", "샴푸 1L", 10000, "2026-01-25", "2026-01-30", "충포장", "", "", "", "보류", "자재 1/22") _
    )
...
Private Function ReadRawData(ByRef items() As ProductionItem, ByRef itemCount As Long) As Boolean
    ...
    For i = 2 To lastRow
        With items(i - 1)
            ...
            ' 이동일 파싱 (사용자 입력)
            If IsDate(ws.Cells(i, RAW_COL_TRANSFER).Value) Then
                .TransferDate = CDate(ws.Cells(i, RAW_COL_TRANSFER).Value)
            ElseIf Len(Trim(CStr(ws.Cells(i, RAW_COL_TRANSFER).Value))) > 0 Then
                .TransferDate = ParseDateString(CStr(ws.Cells(i, RAW_COL_TRANSFER).Value))
            Else
                .TransferDate = 0
            End If
            ...
            ElseIf Len(Trim(CStr(ws.Cells(i, RAW_COL_DUEDATE).Value))) > 0 Then
                .DueDate = ParseDateString(CStr(ws.Cells(i, RAW_COL_DUEDATE).Value))
            Else
                .DueDate = 0  ' 1코드 세트품은 납기 없을 수 있음
            End If
    ...
    For i = 1 To itemCount
        With items(i)
            ...
            ElseIf .ProductionEndDate <= .DueDate - 1 Then
                .DueCheckStatus = "OK"  ' 납기 하루 전까지 완료
            ElseIf .ProductionEndDate = .DueDate Then
                .DueCheckStatus = "위험"  ' 당일 완료 (당일직납)
            Else
                .DueCheckStatus = "불가"  ' 납기 초과
            End If
        End With
NextCalc:
    Next i
...
Private Function GetEffectiveCapacity(vendorName As String, urgency As String, isSetItem As Boolean) As Long
    ...
    Select Case urgency
        Case "긴급"
            capaHours = HOURS_BASIC + HOURS_OVERTIME + (HOURS_SATURDAY / 5)
        Case "급함"
            capaHours = HOURS_BASIC + HOURS_OVERTIME
        Case Else
            capaHours = HOURS_BASIC
    End Select
        ...
        With items(i)
            If .RowNum > 0 Then
                ...
                If .ProductionStartDate > 0 Then
                    ...
                    ws.Cells(.RowNum, RAW_COL_PRODSTART).NumberFormat = "yyyy-mm-dd"
                End If
                
                If .ProductionEndDate > 0 Then
                    ws.Cells(.RowNum, RAW_COL_PRODEND).Value = .ProductionEndDate
                    ws.Cells(.RowNum, RAW_COL_PRODEND).NumberFormat = "yyyy-mm-dd"
                End If
...
Private Sub PlaceItemOnCalendar(ByRef ws As Worksheet, ByRef result As AllocationResult, _
    ...
    Do While remainingQty > 0
        ...
        ' 납기준수 상태 표시
        If result.Item.DueCheckStatus = "불가" Then
            cellText = cellText & vbLf & "납기초과"
        ElseIf result.Item.DueCheckStatus = "위험" Then
            cellText = cellText & vbLf & "당일직납"
        End If
        ...
        If result.Item.DueCheckStatus = "불가" Or result.Item.DueCheckStatus = "위험" Then
            commentText = "[ 납기 " & result.Item.DueCheckStatus & " ] " & vbLf
            commentText = commentText & "납기: " & Format(result.Item.DueDate, "m/d") & vbLf
            commentText = commentText & "완료예정: " & Format(result.Item.ProductionEndDate, "m/d") & vbLf
            commentText = commentText & "────────────────" & vbLf
        End If
        
        If result.HasWarning Then
            commentText = commentText & "[ 경고 ] " & result.WarningMessage & vbLf & "────────────────" & vbLf
        End If
...
Private Function CreateUnassignedSection(ws As Worksheet, ByRef results() As AllocationResult, _
    ...
    ' 헤더
    ws.Range(ws.Cells(startRow, 1), ws.Cells(startRow, 9)).Value = _
        Array("품번", "품명", "수량", "이동일", "납기", "사유", "납기준수", "대안 업체", "조치")
    ...
    For i = 1 To itemCount
        If Not results(i).Success And Len(results(i).FailReason) > 0 And results(i).FailReason <> "HOLD" Then
            ...
            ' 납기준수
            ws.Cells(startRow, 7).Value = results(i).Item.DueCheckStatus
            
            ' 대안 업체
            ws.Cells(startRow, 8).Value = results(i).AlternativeVendors
            
            ' 조치
            ws.Cells(startRow, 9).Value = "[ ] 억지로배정 / [ ] 내부생산 / [ ] 납기협의"
            
            ' 스타일
            ws.Range(ws.Cells(startRow, 1), ws.Cells(startRow, 9)).Interior.Color = RGB(254, 242, 242)
            ws.Cells(startRow, 6).Font.Color = RGB(185, 28, 28)
            ws.Cells(startRow, 6).Font.Bold = True
            ws.Cells(startRow, 8).Font.Color = RGB(22, 101, 52)
...
Private Function CreateVendorPlanSheet(ByRef vendor As VendorInfo, _
    ...
    ws.Cells(currentRow, 1).Value = "업데이트시각: " & Format(Now, "yyyy-mm-dd hh:mm") & _
                                    "  |  총 " & vendorItemCount & "건"
    With ws.Cells(currentRow, 1)
        .Font.Size = 11
        .Font.Color = RGB(100, 116, 139)
    ...
    Dim dueWarningCount As Integer: dueWarningCount = 0
    ...
    For i = 1 To vendorItemCount
        ...
        If vendorItems(i).VendorChanged Then
            ...
            hasChanges = True
        End If
        If vendorItems(i).DueCheckStatus = "위험" Or vendorItems(i).DueCheckStatus = "불가" Then
            dueWarningCount = dueWarningCount + 1
        End If
    Next i
    
    If hasChanges Or dueWarningCount > 0 Then
        ...
        If dueWarningCount > 0 Then changeText = changeText & "납기주의: " & dueWarningCount & "건"
    ...
    ws.Range(ws.Cells(headerRow, 1), ws.Cells(headerRow, 9)).Value = _
        Array("이동일", "품번", "품명", "수량", "납기", "납기준수", "공정", "긴급도", "비고")
    
    With ws.Range(ws.Cells(headerRow, 1), ws.Cells(headerRow, 9))
        .Font.Bold = True
        .Font.Size = 11
        .Font.Color = RGB(255, 255, 255)
    ...
    For i = 1 To vendorItemCount
        With vendorItems(i)
            ...
            ' 납기준수
            ws.Cells(currentRow, 6).Value = .DueCheckStatus
            Select Case .DueCheckStatus
                Case "OK"
                    ws.Cells(currentRow, 6).Font.Color = RGB(22, 101, 52)
                Case "위험"
                    ws.Cells(currentRow, 6).Font.Color = RGB(194, 65, 12)
                    ws.Cells(currentRow, 6).Font.Bold = True
                Case "불가"
                    ws.Cells(currentRow, 6).Font.Color = RGB(185, 28, 28)
                    ws.Cells(currentRow, 6).Font.Bold = True
            End Select
            
            ' 공정
            ws.Cells(currentRow, 7).Value = .ProcessType
            If Len(.SpecialProcess) > 0 Then
                ws.Cells(currentRow, 7).Value = .ProcessType & " (" & .SpecialProcess & ")"
            End If
            
            ' 긴급도
            ws.Cells(currentRow, 8).Value = .Urgency
            If .Urgency = "긴급" Then
                ws.Cells(currentRow, 8).Font.Color = RGB(185, 28, 28)
                ws.Cells(currentRow, 8).Font.Bold = True
            ElseIf .Urgency = "급함" Then
                ws.Cells(currentRow, 8).Font.Color = RGB(194, 65, 12)
                ws.Cells(currentRow, 8).Font.Bold = True
            End If
            
            ' 비고 (변경 상태)
            Dim noteText As String: noteText = ""
            If .Status = "신규" Then
                noteText = "추가"
            ElseIf .VendorChanged Then
                noteText = "변경"
            End If
            ws.Cells(currentRow, 9).Value = noteText
            If Len(noteText) > 0 Then
                ws.Cells(currentRow, 9).Font.Color = RGB(220, 38, 38)
                ws.Cells(currentRow, 9).Font.Bold = True
            End If
            
            ' 행 스타일
            Dim rowBgColor As Long
            
            If .DueCheckStatus = "불가" Then
                rowBgColor = RGB(254, 226, 226)  ' 빨간 배경
            ElseIf .DueCheckStatus = "위험" Then
                rowBgColor = RGB(254, 243, 199)  ' 주황 배경
            ElseIf .TransferDate <= Date + FIXED_DAYS Then
                rowBgColor = RGB(239, 246, 255)  ' 연한 파랑 (확정)
            ElseIf Len(noteText) > 0 Then
                rowBgColor = RGB(254, 243, 199)  ' 연한 노랑 (변경)
            Else
            ...
            ws.Range(ws.Cells(currentRow, 1), ws.Cells(currentRow, 9)).Interior.Color = rowBgColor
            
            ' 이동일 변경 시 구분선
            If prevTransferDate > 0 And .TransferDate <> prevTransferDate Then
                ws.Range(ws.Cells(currentRow, 1), ws.Cells(currentRow, 9)).Borders(xlEdgeTop).LineStyle = xlContinuous
                ws.Range(ws.Cells(currentRow, 1), ws.Cells(currentRow, 9)).Borders(xlEdgeTop).Weight = xlMedium
                ws.Range(ws.Cells(currentRow, 1), ws.Cells(currentRow, 9)).Borders(xlEdgeTop).Color = RGB(203, 213, 225)
            End If
    ...
    ' 테이블 테두리
    With ws.Range(ws.Cells(headerRow, 1), ws.Cells(currentRow - 1, 9))
        .Borders(xlEdgeLeft).LineStyle = xlContinuous
        .Borders(xlEdgeLeft).Weight = xlMedium
    ...
    ws.Columns(6).ColumnWidth = 10  ' 납기준수
    ws.Columns(7).ColumnWidth = 16  ' 공정
    ws.Columns(8).ColumnWidth = 8   ' 긴급도
    ws.Columns(9).ColumnWidth = 8   ' 비고
